...
public class ConfiguracionMascota implements Initializable{
	...
	private static final String[] g_sanguineo = {"DEA-1.1.", "DEA-1.2.", "DEA-3", "DEA-4", "DEA-5", "DEA-6", "DEA-7", "DEA-8"};

	@FXML
	DatePicker f_nacimiento;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		sanguineo.getItems().addAll(g_sanguineo);

		if (selectedPet == null) {
			return;
		}

		nombre.setText(selectedPet.getNombre());
		color.setText(selectedPet.getColor());
		raza.setText(selectedPet.getRaza());
		n_chip.setText(selectedPet.getN_chip());
		sexo_mascota.setValue(selectedPet.getSexo());
		...
		sanguineo.setValue(selectedPet.getG_sanguineo());

		DateTimeFormatter df = DateTimeFormatter.ofPattern("dd/MM/yyyy");
		f_nacimiento.setValue(LocalDate.parse(selectedPet.getF_nacimiento(), df));
	...
	public void save(ActionEvent event) throws StreamWriteException, DatabindException, IOException {
		...
		mascota.setN_chip(n_chip.getText());
		mascota.setSexo(sexo_mascota.getValue());
		mascota.setEsterilizado(esterilizada.getValue() == "Si");
		mascota.setG_sanguineo(sanguineo.getValue());
		mascota.setF_nacimiento(f_nacimiento.getValue().format(df));

...
public class HigieneController {
	...
	@FXML
	private TextField ult_bano;

	@FXML
	private TextField ult_cepillado;

	@FXML
	private TextArea producto_nr;

	@FXML
	private TextArea ult_desp;
	...
	public void selectProfile(int id) throws Exception {
		...
		ult_bano.setText(higiene.getBano());

		ult_cepillado.setText(higiene.getCepillado());

		ult_desp.setText(desparasitadoToString(mascota));

		producto_nr.setText(productosToString(mascota));
	...
	private String productosToString(Mascota mascota) {
		...
		ArrayList<String> productosMascota = mascota.getHigiene().getProducto_nr();
